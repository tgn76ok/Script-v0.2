...
def encontra_placas(caminho_imagem):
    ...
    # suaviza a imagem
    suavizada = cv2.GaussianBlur(original_cinza, (9, 9), 0)
    ...
    dilatacao_final = cv2.dilate(fechamento3, kernel, iterations=1)
    cv2.imwrite(diretorio + "11 - dilatacao_final.jpg", dilatacao_final)

    # Encontra contorno das áreas segmentadas após abertura.
    contours = cv2.findContours(dilatacao_final, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[0]
...
encontra_placas(rootdir+"PVX0095.jpg")
